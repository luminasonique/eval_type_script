...
class UserMapper{
   ...
   static toPersistance(user: User): MongoUser.UserDocument{
...
export class MongoUserRepository implements IUserRepository {
    ...
    async create(user: User): Promise<void> {
        ...
        const existingDocumment = await this.model.findById(user.props.id)
        if(existingDocumment) throw new Error('User already exists')


        await UserMapper.toPersistance(user).save()
